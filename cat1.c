...
// Функция для чтения файла и вывода его содержимого на экран с учетом флагов
void read_print(char *filename, Flags flags) {
  FILE *fp;
  int ch = 0;
  fp = fopen(filename, "r");
  ...
  char lastC = '\n';
  int count_empty = 0;
  while ((ch = fgetc(fp)) != EOF) {  // fgets считывает посимвольно
...
Flags parse_flags(int argc, char *argv[]) {
  Flags flags = {0, 0, 0, 0, 0, 0, 0, 0};  // Инициализируем все флаги нулями
  int opt;
  int option_index;
